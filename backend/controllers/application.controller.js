...
export const employerGetAllApplications = asyncError(
  async (req, res, next) => {
    const { role } = req.user;
    if (role === "JobSeeker") {
      return next(
...
export const jobseekerGetAllApplications = asyncError(
  async (req, res, next) => {
    const { role } = req.user;
    if (role === "Employer") {
      return next(
...
export const jobseekerDeleteApplication = asyncError(
  async (req, res, next) => {
    const { role } = req.user;
    if (role === "Employer") {
      return next(
